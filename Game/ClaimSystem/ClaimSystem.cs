...
namespace Chromonia.ClaimSystem;
...
public class ClaimSystem(Node2D playfield, Node2D maskRoot, Line2D perimeterLine, Arrow.Arrow arrow)
{
    ...
    {
        var poly1 = GeometryUtils.GeometryUtils.BuildPolygon(perimeter, startSeg, hitSegmentIndex, activeArray, forward: true);
        var poly2 = GeometryUtils.GeometryUtils.BuildPolygon(perimeter, startSeg, hitSegmentIndex, activeArray, forward: false);

        float area1 = GeometryUtils.GeometryUtils.GetPolygonArea(poly1);
        float area2 = GeometryUtils.GeometryUtils.GetPolygonArea(poly2);

...
public class PlayerController(Arrow.Arrow arrow, Line2D drawingLine)
{
    ...
    private bool IsOnSelfLine()
    {
        ...
        for (int i = 0; i < _activeLine.Count - 3; i++)
            if (GeometryUtils.DistanceToSegment(arrow.Position, _activeLine[i], _activeLine[i + 1]) < 0.05f)
                return true;
    ...
    private void HandleAdvancing(Vector2 velocity, Vector2 inputDir, Vector2[] perimeter)
    {
        ...
        {
            var inter = GeometryUtils.SegmentIntersectsSegment(moveA, moveB, perimeter[i], perimeter[i + 1]);
        ...
        if (!hitPoint.HasValue && _activeLine.Count >= 4)
        {
            for (int i = 0; i < _activeLine.Count - 3; i++)
            {
                var inter = GeometryUtils.SegmentIntersectsSegment(moveA, moveB, _activeLine[i], _activeLine[i + 1]);
                if (!inter.HasValue) continue;
